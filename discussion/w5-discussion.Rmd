...
```{r}
fish_recipe <- recipe(habitat ~ shape_impute + food_impute + shallow_impute +
                       deep_impute + vulnerability + length_impute + 
                       trophic_level_impute, data = train) %>%
...
```{r, eval = FALSE}
knn_cv_tine <- knn_workflow %>% 
  tune_grid(resamples = cv_folds, grid = 5) #use cross validation for neighbors
```

...
Set standard set-up for chunk outputs. Load necessary libraries and use `set.seed()` to achieve same random values every time the code is run
...
Additionally, we are going to binary encode these values. This means turning them into a 0 and 1. **This should be a logistic regression, for the purpose of this discussion we'll be conducting a linear regression to see why it does not work.**

```{r}
# Clean and read-in fish data
...
ggplot(fish_clean, aes(habitat)) + #order by count
  geom_bar(fill = 'maroon') + #get counts
  theme_bw() + #bw theme
  labs(title = "Count of Habitat Types", 
       x = "Habitat",
...
# Calculate correlations between numberic variables
fish_clean %>% 
  dplyr::select(shallow_impute, deep_impute, vulnerability, 
                length_impute, trophic_level_impute) %>%  #select numeric cols
...
fish_clean %>% 
  dplyr::select(shallow_impute, deep_impute, 
                vulnerability, length_impute, 
                trophic_level_impute) %>%  #select numeric columns
...
After splitting my data, the first step is to create a recipe indicating the formula for the model and any pre-processing steps I want to preform on my data. Here we will use the `recipes` package. `prep` trains your processing steps using your `recipe`, for example step-imputes which fill in gaps based on averages. There are multiple processing steps being set by prep before baking. `bake` actually applies the parameters that are set by `prep` to your data.
...
I will be predicting habitat based on trophic level, length, minimum depth, maximum depth, vulnerability, body shape, and food source type. We will be normalizing all numeric predictors instead of utilizing `bake` and `prep`.

```{r}
fish_recipe <- recipe(habitat ~ shape_impute + food_impute +
                        shallow_impute + deep_impute + vulnerability +
                        length_impute + trophic_level_impute,
                      data = train) %>% 
...
`set_engine()` is the calling the function that you're using from another package. In this case, we're using linear regressions from base R. Say we wanted to use a generalized linear model, we'd have to load the `glm` package to use it as the engine. For linear regression alone, there are 14 engines such as Bayesian linear regression, and mixed models.
...
```{r, eval = FALSE}
# Run fit on workflow and training data
train_fit <- fit(lm_workflow, train) #fit the lm model to the training set

# Prints out beta values for the linear regression
train_fit
```

```{r}
# Run model with training data (optional. not a hw requirement)
train_predict <- predict(object = train_fit,
                         new_data = train) %>% # predict the training set
...
test_predict <- predict(object = train_fit,
                        new_data = test) %>% 
  bind_cols(test) #bind to testing columns

# Predicitons are expected to be bad because it should have been a logit regression, we'll see this in the following steps when metrics are run
head(test_predict)
```

#### Model Metrics and Evaluation

Finally, we will calculate some metrics for performance, using the `yardstick` package.
...
**Analyze results**: `rmse` is not technically a standard error, but can be interpreted similarly. It is highly dependent on the scale of your data (view definition in w1-lab). `mae` takes the mean of the absolute value of the observed minus actual (I think). Typically `mae` is lower than `rmse`, but it's not as pivotal for model interpretation than `rmse` and `rsq`.

If you have a categorical outcome, you almost never want to run linear regression. For this reason, the model evaluation has produced poor results.

...
```{r}
fish_clean <- read_csv(here("discussion", "data", "w2-fish-clean.csv")) %>% mutate(habitat = as.factor(habitat))
...
rf_model <- rand_forest(mtry = tune(),
                  trees = tune()) %>% 
  # ranger is the model from pakcage ranger loaded in
  set_engine("ranger") %>% 
  set_mode("classification") #can do class or regression
```
...
**NOTE:** Under "Main Arguments" you can see the parameters are set to tune.
...
```{r, eval = FALSE}
# takes long to run, so commented out here
...
```{r, include=FALSE}
# read in results instead of running tune_grid
load(file = here("discussion", "data",  "w6-rf_tune.rda"))
```
...
**NOTE:** For the predictors there's a noticable increase in AUC and accuracy as randomly selected predictors increase. There is not a notable pattern based on number of trees. \#### Finalize workflow

```{r}
#get metrics for best random forest model
...
**NOTE:** For `boosted_trees` you can later the `learn_rate` this determines how much the model learns based on the tree before it.
...
```{r, eval = FALSE}
# read-in to avoid running slow model
...
```{r, include=FALSE}
load(file = here::here("discussion","data",  "w6-xgb_tune.rda"))
```
...
**NOTE:** Accuracy and AUC both improve as the value of `learn_rate` increases. Again the number of trees also has no clear pattern, indicating the learning rate is more important in creating an optimized model.

#### Finalize workflow

```{r}
#get metrics for best random forest model
xgb_best <- show_best(xgb_cv_tune, n = 1, metric = "roc_auc") 
...
```{r}
# finalize work flows, combine previous workflow w/best inputs
rf_final <- finalize_workflow(rf_workflow, select_best(rf_cv_tune, metric = "roc_auc")) #random forest

xgb_final <- finalize_workflow(xgb_workflow,
                               select_best(xgb_cv_tune, metric = "roc_auc")) #boosted trees
...
**NOTE**: The `boosted_tree()` model works best in this case, it has a slightly higher accuracy than the random forest method (0.898 vs 0.888)

...
# create data with binary outcome, and we'll use two variables to generate
sim_data <- tibble(
  x1 = rnorm(40), #using rnorm to draw 40 points from normal distribution
  x2 = rnorm(40), #each point has x1,x2 coord
  y = factor(rep(c(-1, 1), 20))) %>% 

  # if y=1, change x1 to be x1+1.5 (else keep value of x1)
  mutate(x1 = ifelse(y==1, x1 + 1.5, x1),
         x2 = ifelse(y==1, x2 + 1.5, x1))
...
ggplot(sim_data, aes(x1, x2, color = y)) +
  geom_point() +
  theme_minimal() +
  scale_color_manual(values = c("orchid2", "firebrick"))
...
```{r svm_spec}
#Create linear SVM model specification (using polynomial kernel)
# trying to predict linear relationship
...
**NOTE:** The margin (purple line from lecutre) can have varying widths. A wider margin means we're allowing more missclassifications. Remember by allowing miss classification, our model becomes more generalized and potentially better at predicting on new data. I THINK this is the `cost` parameter.
...
svm_linear_wf <- workflow() %>% 
  add_model(svm_linear_spec %>% 
  set_args(cost_tune())) %>% #will override the cost=10 we set earlier
  add_formula(y~.)

set.seed(1)

# set folds (idk what strata is)
...
**NOTE:** Subtract training error from 100 to get the accuracy.

```{r sim_test}
#Create a small test data set (set seed to different value)
set.seed(2)
...
```{r augment}
# create confusion matrix after predicting on test data
augment(svm_linear_fit, new_data = sim_test) %>% 
...
sim_data2 %>% 
  ...
  theme_minimal() +
  scale_color_manual(values = c("seagreen", "darkorange1"))
```
...
augment(svm_rbf_fit, new_data = sim_test2) %>% 
  conf_mat(truth = y, estimate = .pred_class)
```

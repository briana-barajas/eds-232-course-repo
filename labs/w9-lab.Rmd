...
# check how many times each cover type appears in the data set
plyr::count(cover_type$Cover_Type) %>% gt()

# count for soil_type15
plyr::count(cover_type$Soil_Type_15) %>% gt()
...
cover_type <- cover_type %>% 
  ...
  # remove soil type 15, which has no variation
  select(-c(Soil_Type_15))

```

**ANS:** It appears the soil types are mutually exclusive. The data set could be pivoted to change this into a single column, although I know from prior experience running regressions that this will not affect the outcome.
...
# create svm model specification
svm_spec <- svm_poly(cost = tune(),
                     degree = tune()) %>% 
  set_mode("classification") %>% 
  set_engine("kernlab", nthread = 2)
...
**ANS:** The same recipe can be used for the support vector machines, and random forest model. Since all the variables being tested are numeric, it's not necessary to create dummy variables with `step_dummy()`.

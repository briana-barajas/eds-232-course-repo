...
**ANS:** The plot above is unable to efficiently demonstrate the relationship between pumpkin price, and the day of purchase. Finding the average daily pumpkin price would be a better way to visualize this data. Taking the average could also help distinguish the points on days that are clustered together in the current plot (i.e. points just below day 275).
...
**ANS:** Based on the variables I selected, `variety` is the most highly correlated with price. One of the variety types is "pie type", so there is a potential price discrepancy since these pies are grown for consumption. There are also types "miniature" and "heirloom", which can have size and quality differences that affect their pricing.
...
**ANS:** Based on the R-squared value, `package` type only explains about 49.5% of the variable in price. Since the `package` types are in binned categories, the linear regression does not create a great fit. This can be better visualized in the scatter plot above, as the line does not seem to fit much of the data.
